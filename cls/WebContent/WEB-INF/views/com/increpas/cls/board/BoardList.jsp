...
<body>
	...
	<c:if test="${empty SID}">
		...
		<div class="w3-col w3-margin-top w3-padding w3-card-4">
			...
			<form id="frm" name="frm"  class="w3-col w3-border-left w3-border-right w3-margin-bottom">
					<input type="hidden" id="bbno" name="bno"> 
					<input type="hidden" id="nowPage" name="nowPage" value="${PAGE.nowPage}"> 
	
	<c:forEach var="data" items="${LIST}">
				<div class="w3-col w3-center w3-border-bottom w3-hover-light-grey listItems">
					<div class="w3-col m1 w3-border-right" id="b${data.bno}">${data.bno}</div>
		...
		<c:if test="${data.cnt != 0}">
					...
					<div class="w3-col m1">${data.bclick}</div>
				</div>
	</c:forEach>
